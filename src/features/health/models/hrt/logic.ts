...
export const T_ESTERS: ReadonlySet<Ester> = new Set<Ester>([
  Ester.T,
  Ester.TC,
  Ester.TE,
  Ester.TU,
 ...
 */
function _maxDailyDose(
  events: DoseEvent[],
  nowH: number,
  days: number,
  pred: (e: DoseEvent) => boolean,
): number {
...
function _injectionWeeklyRate(
  ...
  for (let i = 1; i < fam.length; i++)
    gaps.push(fam[i].timeH - fam[i - 1].timeH)
  gaps.sort((a, b) => a - b)
 ...
 */
export function getDoseAdvisory(
  events: DoseEvent[],
  nowH: number = Date.now() / (1000 * 60 * 60),
): DoseAdvisory | null {
  if (!events.length) return null

  const isE2 = (e: DoseEvent) =>
    !isTestosteroneEster(e.ester) && e.ester !== Ester.CPA
  const isT = (e: DoseEvent) => isTestosteroneEster(e.ester)

  // Daily-dosed families: heaviest single-day total in the trailing 14 days.
  const cpaPerDay = _maxDailyDose(
    events,
    nowH,
    14,
    (e) => e.ester === Ester.CPA,
  )
  const e2DailyPerDay = _maxDailyDose(
    events,
    nowH,
    14,
    (e) => isE2(e) && _isDailyRoute(e.route),
  )
  const tDailyPerDay = _maxDailyDose(
    events,
    nowH,
    14,
    (e) => isT(e) && (e.route === Route.gel || e.route === Route.patchApply),
  )

  // Injected families: cadence-inferred weekly rate, plus a single-dose guard for E2.
  const e2InjWk =
    _injectionWeeklyRate(
      events,
      nowH,
      (e) => isE2(e) && e.route === Route.injection,
    ) ?? 0
  const tInjWk =
    _injectionWeeklyRate(
      events,
      nowH,
      (e) => isT(e) && e.route === Route.injection,
    ) ?? 0
  let e2InjSingle = 0
  for (const e of events) {
    if (
      isE2(e) &&
      e.route === Route.injection &&
      nowH - e.timeH >= 0 &&
      nowH - e.timeH <= 30 * 24
    ) {
  ...
  const e2Ratio = Math.max(
    e2DailyPerDay / DOSE_CEILING.e2DailyPerDay,
    e2InjWk / DOSE_CEILING.e2InjPerWeek,
    e2InjSingle / DOSE_CEILING.e2InjSingle,
  )
  const tRatio = Math.max(
    tDailyPerDay / DOSE_CEILING.tDailyPerDay,
    tInjWk / DOSE_CEILING.tInjPerWeek,
  )
 ...
 */
export function getHormoneLevelAdvisory(
  results: LabResult[],
): HormoneLevelAdvisory | null {
  ...
  const { e2LowPgMl, e2HighPgMl, tLowNgDl, tHighNgDl } =
    HORMONE_LEVEL_THRESHOLDS
...
export const CALIBRATION_HISTORY_MODES: readonly CalibrationHistoryMode[] = [
  'forward',
  'retrospective',
]

/** Map legacy stored method ids onto the current estimator set. */
export function normalizeCalibrationMethod(
  raw: string | null | undefined,
): CalibrationMethod {
  if (raw === 'off' || raw === 'ekf' || raw === 'ou_kalman' || raw === 'mipd')
    return raw
...
export function computeCalibrationPoints(
  ...
  const getNearestConc_E2 = (timeH: number): number | null => {
    ...
    const idx =
      Math.abs(sim.timeH[high] - timeH) < Math.abs(sim.timeH[low] - timeH)
        ? high
        : low
...
// well-spaced labs.
const CAL_PRIOR_SD_AMP = 0.5 // ≈ ±65% amplitude
...
function ekfFit(
  ...
  for (let i = 0; i < logObs.length; i++) {
    ...
    const a00 = 1 - kg0 * H0,
      ...
      a10 = -kg1 * H0,
      a11 = 1 - kg1 * H1
    const np00 = a00 * p00 + a01 * p10,
      np01 = a00 * p01 + a01 * p11
    const np10 = a10 * p00 + a11 * p10,
      np11 = a10 * p01 + a11 * p11
    p00 = np00
    p01 = np01
    p10 = np10
    p11 = np11
...
function mipdFit(
  ...
  for (let iter = 0; iter < 16; iter++) {
    ...
    A11 = precK
    let b0 = -precA * a,
      b1 = -precK * k // prior pulls toward (0,0)
...
function ouCalibrate(
  ...
  const zf = Array.from({ length: n }, () => 0),
    Pf = Array.from({ length: n }, () => 0),
    zPred = Array.from({ length: n }, () => 0),
    Ppred = Array.from({ length: n }, () => 0)
  for (let i = 0; i < n; i++) {
    if (i === 0) {
      zPred[i] = 0
      Ppred[i] = Pstat
  ...
  const factorFn = (t: number): number => {
    ...
    if (forward) {
      ...
      const D = labTimes[j + 1] - labTimes[j],
        s = t - labTimes[j]
      const sh = Math.sinh(theta * D) || 1 // OU bridge conditional mean
      z =
        (Math.sinh(theta * (D - s)) * zs[j] +
          Math.sinh(theta * s) * zs[j + 1]) /
        sh
    }
    ...
    factorFn =
      historyMode === 'forward'
        ? piecewiseFactor(
            field,
            labTimes,
            fit.snapshots.map((s) => ({ a: s.a + 0.5 * s.paa, k: s.k })),
          )
...
const EsterInfo = {
  ...
  [Ester.EB]: { name: 'Estradiol Benzoate', mw: 376.5 },
  [Ester.EV]: { name: 'Estradiol Valerate', mw: 356.5 },
  ...
  [Ester.TC]: { name: 'Testosterone Cypionate', mw: 412.6 },
  [Ester.TE]: { name: 'Testosterone Enanthate', mw: 400.59 },
  [Ester.TU]: { name: 'Testosterone Undecanoate', mw: 456.7 },
}
...
const T_DepotPK = {
  Frac_fast: { [Ester.TC]: 0.35, [Ester.TE]: 0.4, [Ester.TU]: 0.1 },
...
const T_EsterPK = {
  k2: { [Ester.TC]: 0.2, [Ester.TE]: 0.2, [Ester.TU]: 0.2 },
}
...
const TwoPartDepotPK = {
  ...
  k1_fast: {
    [Ester.EB]: 0.144,
    [Ester.EV]: 0.0216,
    [Ester.EC]: 0.005035046,
    [Ester.EN]: 0.001,
    [Ester.EU]: 0.006,
    [Ester.E2]: 0.5,
  }, // Added non-zero k1 for E2
  k1_slow: {
    [Ester.EB]: 0.114,
    [Ester.EV]: 0.0138,
    [Ester.EC]: 0.004510574,
    [Ester.EN]: 0.005,
    [Ester.EU]: 0.0022,
    [Ester.E2]: 0,
  },
...
export function applyPKOverrides(params: PKCustomParams | null): void {
  _activePKParams = params
    ? { ...DEFAULT_PK_PARAMS, ...params }
    : { ...DEFAULT_PK_PARAMS }
}
...
export function getBioavailabilityMultiplier(
  ...
  if (isTestosteroneEster(ester)) {
    switch (route) {
      ...
      case Route.gel: {
        const siteIdx = Math.min(
          GEL_SITE_ORDER.length - 1,
          Math.max(0, Math.round(extras[ExtraKey.gelSite] ?? 0)),
        )
    ...
    case Route.sublingual: {
      ...
      if (extras[ExtraKey.sublingualTheta] !== undefined) {
        ...
        const tierIdx = Math.min(
          SL_TIER_ORDER.length - 1,
          Math.max(0, Math.round(extras[ExtraKey.sublingualTier]!)),
        )
...
function resolveParams(event: DoseEvent): PKParams {
  const defaultK3 =
    event.route === Route.injection
      ? _activePKParams.e2_kClearInj
      : _activePKParams.e2_kClear
  ...
  if (isTestosteroneEster(event.ester)) {
    const tK3 =
      event.route === Route.injection
        ? _activePKParams.t_kClearInj
        : _activePKParams.t_kClear
    switch (event.route) {
      case Route.injection: {
        ...
        const k2 = (T_EsterPK.k2 as any)[event.ester] ?? 0.2
        const F = getBioavailabilityMultiplier(
          Route.injection,
          event.ester,
          extras,
        )
      ...
      case Route.patchApply: {
        ...
        const rateMGh =
          typeof releaseRateUGPerDay === 'number' &&
          Number.isFinite(releaseRateUGPerDay) &&
          releaseRateUGPerDay > 0
  ...
  switch (event.route) {
    case Route.injection: {
      const Frac_fast =
        (TwoPartDepotPK.Frac_fast as Partial<Record<Ester, number>>)[
          event.ester
        ] ?? 0.5
      const k1_fast =
        ((TwoPartDepotPK.k1_fast as Partial<Record<Ester, number>>)[
          event.ester
        ] ?? 0.1) * CorePK.depotK1Corr
      const k1_slow =
        ((TwoPartDepotPK.k1_slow as Partial<Record<Ester, number>>)[
          event.ester
        ] ?? 0.01) * CorePK.depotK1Corr
      const k2 =
        (EsterPK.k2 as Partial<Record<Ester, number>>)[event.ester] ?? 0
      const F = getBioavailabilityMultiplier(
        Route.injection,
        event.ester,
        extras,
...
function _separateRates(
  ...
  for (let i = 0; i < 6; i++) {
    let moved = false
    if (Math.abs(k3 - k1) < eps) {
      k3 = k1 + (k3 >= k1 ? eps : -eps)
      moved = true
    }
    if (Math.abs(k3 - k2) < eps) {
      k3 = k2 + (k3 >= k2 ? eps : -eps)
      moved = true
    }
    if (!moved) break
...
function oneCompAmount(tau: number, doseMG: number, p: PKParams): number {
  ...
  return (
    ((doseMG * p.F * k1) / (k1 - p.k3)) *
    (Math.exp(-p.k3 * tau) - Math.exp(-k1 * tau))
  )
}
...
function resolvePatchWearH(event: DoseEvent, allEvents: DoseEvent[]): number {
  const remove = allEvents.find(
    (e) => e.route === Route.patchRemove && e.timeH > event.timeH,
  )
      ...
      case Route.patchApply:
        ...
        this.model = (timeH: number) => {
          ...
          if (params.rateMGh > 0) {
            if (tau <= wearH) {
              return (
                (params.rateMGh / params.k3) * (1 - Math.exp(-params.k3 * tau))
              )
            } else {
              const amtRemoval =
                (params.rateMGh / params.k3) *
                (1 - Math.exp(-params.k3 * wearH))
...
function computeMaxLifetimeH(
  ...
  if (route === Route.patchApply) {
    const applyEvent = allEvents.find(
      (e) => e.route === Route.patchApply && e.timeH === eventTimeH,
    )
    const wearH = applyEvent
      ? resolvePatchWearH(applyEvent, allEvents)
      : Number.MAX_VALUE
...
export function runSimulation(
  ...
  const precomputed = sortedEvents
    ...
    .map((e) => {
      ...
      const maxLifetimeH = computeMaxLifetimeH(
        params,
        e.route,
        sortedEvents,
        e.timeH,
  ...
  for (const pc of precomputed) {
    ...
    if (
      pc.maxLifetimeH !== Infinity &&
      endTime - pc.startTimeH > 2 * pc.maxLifetimeH
    )
      continue
  ...
  const allTimes = Array.from(
    new Set([...gridTimes, ...eventTimes, ...periEventTimes]),
  ).sort((a, b) => a - b)
  ...
  for (let i = 0; i < allTimes.length; i++) {
    ...
    while (
      activateIdx < precomputed.length &&
      precomputed[activateIdx].startTimeH <= t
    ) {
...
export function interpolateConcentration(
  ...
  if (hour >= sim.timeH[sim.timeH.length - 1])
    return sim.concPGmL[sim.concPGmL.length - 1]
...
export function interpolateConcentration_E2(
  sim: SimulationResult,
  hour: number,
): number | null {
  if (!sim.timeH.length) return null
  ...
  if (hour >= sim.timeH[sim.timeH.length - 1])
    return sim.concPGmL_E2[sim.concPGmL_E2.length - 1]
...
export function interpolateConcentration_CPA(
  sim: SimulationResult,
  hour: number,
): number | null {
  if (!sim.timeH.length) return null
  ...
  if (hour >= sim.timeH[sim.timeH.length - 1])
    return sim.concPGmL_CPA[sim.concPGmL_CPA.length - 1]
...
export function interpolateConcentration_T(
  sim: SimulationResult,
  hour: number,
): number | null {
  ...
  if (hour >= sim.timeH[sim.timeH.length - 1])
    return sim.concNGdL_T[sim.concNGdL_T.length - 1]
...
): Promise<{ data: string; password: string }> {
  const password =
    customPassword ||
    buffToBase64(window.crypto.getRandomValues(new Uint8Array(12)))
...
export async function decryptData(
  ...
  try {
    ...
    const iterations =
      typeof bundle.iter === 'number' && bundle.iter > 0 ? bundle.iter : 100000
