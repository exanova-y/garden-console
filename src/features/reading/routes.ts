...
async function refreshAll(env: Env): Promise<Record<string, number>> {
    ...
    try {
      result[connector.provider] =
        connector.provider === 'google'
          ? await refreshGoogle(connector, env)
          : await refreshFeedly(connector, env)
      await env.DB.prepare(
...
export async function handleReading(
  ...
  if (path === '/api/reading/refresh' && request.method === 'POST')
...
export async function refreshReading(env: Env): Promise<void> {
